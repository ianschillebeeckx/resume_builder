...
Given a job posting (saved locally from a URL) and the CV database, generate a tailored 2-page LaTeX resume. Do not restrict to 1 page — use the space to fully represent relevant experience. The generation process:
...
When the user says "mark applied" or similar, before starting the next resume:
1. Add the application to `data/applications.yaml` (company, role, source, date, resume path, status, notes)
2. Stage and commit all files (JD, resume .tex/.pdf, application log, any CV updates)
3. Confirm commit before moving on to the next role
...
- **Title Calibration**: When the target role is Director-level (i.e. below VP), downgrade CareDx titles from "VP" to "Sr. Director" on the resume. Keep the CV database unchanged — this is a presentation-layer adjustment only.
